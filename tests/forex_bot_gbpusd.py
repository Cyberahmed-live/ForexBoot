...
def calculate_sl_tp(price, direction, stop_loss_pips, take_profit_pips, symbol):
    ...
    required_sl_distance = max(stop_loss_pips * point, min_stop_distance) + (point * SL_PIPS)  # dodajemy dodatkowy margines
    required_tp_distance = max(take_profit_pips * point, min_stop_distance) + (point * TP_PIPS) # zwiększamy TP
